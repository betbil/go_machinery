...
func startServer(funcOk bool) (*machinery.Server, error) {
	cnf := &config.Config{
...
func send() error {
	...
	server, err := startServer(false) //send true worker false olduğunda hata oluyor tasklar register olmuyor
	if err != nil {
		return err
	}
	...
	*/
	...
	/*
		...
		if err == nil {
			...
			return fmt.Errorf("Error should not be nil if task panicked")
		}
